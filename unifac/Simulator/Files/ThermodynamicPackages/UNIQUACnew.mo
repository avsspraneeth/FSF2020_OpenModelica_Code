...
model UNIQUAC
   ...
   Simulator.Files.ThermodynamicPackages.UNIQUACnew.flag gma(Nc=Nc,C=C,x_pc=x_pc,P=P,T=T,Pdew=Pdew,Pbubl=Pbubl,Pvap_c=Pvap_c);
  ...
  equation
    Cpres_p = zeros(3);//done//verified
    Hres_p = zeros(3);//done//v
    Sres_p = zeros(3);//done//v
   
     for i in 1:Nc loop
      
     rho_c[i] = Simulator.Files.ThermodynamicFunctions.Dens(C[i].LiqDen, C[i].Tc, T, P) * 1E-3;//done//verified
    end for;
   gma.rho_c=rho_c;
   for i in 1:Nc loop
      philiqbubl_c[i] = 1;//done//v
      phivapdew_c[i] = 1;//done//v
    end for;
   
   for i in 1:Nc loop
      Pvap_c[i] = Simulator.Files.ThermodynamicFunctions.Psat(C[i].VP, T);//done//v
    end for;
  // (xliqdew_c,gmanew_c, gmabubl_c, gmadew_c) = flag(Nc,P,T,x_pc,Pdew,rho_c,Pvap_c,C);
   xliqdew_c=gma.xliqdew_c;
   gmanew_c=gma.gmanew_c;
   gmabubl_c=gma.gmabubl_c;
   gmadew_c=gma.gmadew_c;
  // xliqdew_c[i]=0;// from model//hta
   // gmanew_c= Gma.gma_c; // from model//hta
  
   
   // for i in 1:Nc loop
   //   gmabubl_c[i] = GmaBubl.gma_c[i]; //from model//hta
  //    gmadew_c[i] = GmaDew.gma_c[i];//from model//hta
  //
  //  end for;
    
    for i in 1:Nc loop
      K_c[i]= gmanew_c[i] .* Pvap_c[i] ./ P; //done//v
    end for;
  ...
  model flag
   ...
   parameter Integer Nc;
    ...
    Real x_pc[3,Nc];
    Real Pdew;
    Real rho_c[Nc](each unit = "kmol/m^3");
   ...
   Real gmadew_c[Nc](each start = 2.2) "Activity coefficent at dew point";
    ...
    Real A_dew[Nc], B_dew[Nc], C_dew[Nc], D_dew[Nc], E_dew[Nc], F_dew[Nc];
     /// Real tow[Nc, Nc]= Simulator.Files.ThermodynamicFunctions.TowUNIQUAC(Nc, a , T);
    //===========================================================================================================
    //Equation Section
  equation
...
//Function :Poynting_CF is called from the Simulator Package
    for i in 1:Nc loop
      PCF_c[i] = Simulator.Files.ThermodynamicFunctions.PoyntingCF(Nc, C[i].Pc, C[i].Tc, C[i].Racketparam, C[i].AF, C[i].MW, T, P, gma_c[i], Pvap_c[i], rho_c[i]);
      PCFbubl_c[i] = Simulator.Files.ThermodynamicFunctions.PoyntingCF(Nc, C[i].Pc, C[i].Tc, C[i].Racketparam, C[i].AF, C[i].MW, T, Pbubl, gma_c[i], Pvap_c[i], rho_c[i]);
      PCFdew_c[i] = Simulator.Files.ThermodynamicFunctions.PoyntingCF(Nc, C[i].Pc, C[i].Tc, C[i].Racketparam, C[i].AF, C[i].MW, T, Pdew, gma_c[i], Pvap_c[i], rho_c[i]);
    end for;
//Calculation of Fugacity coefficient with Poynting correction
    phil[:] = gma_c[:] .* Pvap_c[:] ./ P .* PCF_c[:];////////////////////////wrong
