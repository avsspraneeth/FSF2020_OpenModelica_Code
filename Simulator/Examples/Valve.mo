...
package Valve
  ...
  model valvewithservice
  extends Modelica.Icons.Example;
    import data = Simulator.Files.ChemsepDatabase;
    parameter data.Nitrogen wat;
    parameter Integer Nc = 1;
    parameter Real x = 2;
    parameter Real y = 1;
    parameter Real OP = 50;
    parameter data.GeneralProperties C[Nc] = {wat};
    Simulator.UnitOperations.Valvenew valve1(Nc = Nc, C = C, x = x, y = y, OP = OP) annotation(
      Placement(visible = true, transformation(origin = {0, 4}, extent = {{-14, -14}, {14, 14}}, rotation = 0)));
    Simulator.Examples.Valve.ms inlet(Nc = Nc, C = C) annotation(
      Placement(visible = true, transformation(origin = {-74, 4}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Simulator.Examples.Valve.ms outlet(Nc = Nc, C = C) annotation(
  ...
  equation
    ...
    inlet.F_p[1] = 35.6972;
//valve1.Pdel=101325;
    valve1.Calmode = "Kvgas";
    valve1.Kvmax = 245;
  end valvewithservice;
  annotation(
